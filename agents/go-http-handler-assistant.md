...
```json
{
  ...
  "payload": {
    "query": "Handler context needed: recent changes scope, main.go location, pkg/handler/ structure, refactoring guide location (/home/node/.claude/docs/coding-guidelines/go-http-handler-refactoring-guide.md), and existing handler patterns."
  }
}
```
...
**Handler Location**:
- All handlers MUST be in `pkg/handler/` package
...
- HTTP handlers: return `libhttp.WithError`
...
- **Important**: Handlers in wrong package, incorrect return types, missing handler package structure
