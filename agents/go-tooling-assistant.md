...
2. **Discover Project Context**
   ...
   - Read existing `/workspace/Makefile` (if present)
   - Read existing `/workspace/tools.go` (if present)
   - Read existing `/workspace/.gitignore` (if present)
   - Read `/workspace/go.mod` to understand dependencies
   - Check for GitHub workflows in `/workspace/.github/workflows/`
   - **Load reference templates** from `~/.claude/templates/`:
     ...
     - **Fallback**: If templates don't exist, read from reference projects:
       - Library: `/home/node/.claude/docs/argument/Makefile`
       - Service: `/home/node/.claude/docs/kafka-topic-reader/Makefile`
       - Recommend creating `~/.claude/templates/` directory for future runs
   - Reference `/home/node/.claude/docs/coding-guidelines/go-makefile-commands.md` for standard patterns
...
7. **Check and Create GitHub Workflows** (Update Mode Only)
   ...
   - Verify presence of standard workflows (or copy from reference project like argument):
     ...
     - **claude.yml**: Interactive Claude assistance
       ...
       - Uses `anthropics/claude-code-action@beta`
   - **Reference workflows**: Use `/home/node/.claude/docs/go-skeleton/.github/workflows/` as template
...
8. **Validate Configuration** (Update Mode Only)
   - Run `cd /workspace/<project> && make ensure` to verify setup
...
**Best Practices:**
- Always use absolute file paths (e.g., `/workspace/Makefile`)
...
- Use `/home/node/.claude/docs/go-skeleton/.github/workflows/` as reference for GitHub workflows
- Use `/home/node/.claude/docs/go-skeleton/` as comprehensive project template reference
...
Changes Applied:
1. Makefile: [Created/Updated] at /workspace/Makefile
   - Added targets: [list]
   - Updated targets: [list]
   - Set default goal: precommit

2. tools.go: [Created/Updated] at /workspace/tools.go
...
- /workspace/Makefile
- /workspace/tools.go
- /workspace/.github/workflows/ci.yml (if created)
- /workspace/.github/workflows/claude-code-review.yml (if created)
- /workspace/.github/workflows/claude.yml (if created)
