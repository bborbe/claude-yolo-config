...
description: Review PR diff against target branch (excludes vendor/node_modules)
---
...
Perform a code review scoped to the PR diff against a target branch. This is identical to `/code-review` but scoped to changed files only, excluding `vendor/` and `node_modules/`.

### Step 0: Determine Target Branch and Generate Diff

Parse the first argument as the target branch name (e.g., `master`, `develop`, `main`). If not provided, default to `master`.

Generate the diff excluding vendor and node_modules:
```bash
git diff origin/<target-branch>...HEAD -- . ':(exclude,glob)**/vendor/**' ':(exclude,glob)**/node_modules/**'
```

Also get the diff stat for overview:
```bash
git diff --stat origin/<target-branch>...HEAD -- . ':(exclude,glob)**/vendor/**' ':(exclude,glob)**/node_modules/**'
```

If the diff is empty, report "No changes to review compared to origin/<target-branch>" and stop.

**IMPORTANT**: All subsequent review steps must focus ONLY on files and lines that appear in this diff. Do not review unchanged code. Read full files for context, but findings must only apply to changed lines.

### Step 1: Parse Mode Argument

Parse the second argument (if present) to determine mode:
- If arg is `short|quick|fast` → **Short mode** (manual review only)
- If arg is `full|comprehensive|complete` → **Full mode** (all 13 agents)
...
**IMPORTANT**: Only report findings for code that was changed in the PR diff (from Step 0). Do not flag issues in unchanged code.

**MANDATORY**: The report MUST always include all three section headers: `### Must Fix (Critical)`, `### Should Fix (Important)`, and `### Nice to Have (Optional)`. If a section has no findings, write "None." under it. Never skip or rename these headers — they are used for automated verdict parsing.
...
- Missing test suite setup (time.Local, format.TruncatedDiff)
...
### Step 7: JSON Verdict Block

**MANDATORY**: After the consolidated report, you MUST append a JSON verdict block on its own line. This is used for automated verdict parsing by pr-reviewer.

Determine the verdict based on your findings:
- `"approve"` — No Must Fix issues AND no Should Fix issues (only Nice to Have or no issues)
- `"request-changes"` — Any Must Fix issues found
- `"comment"` — No Must Fix issues but Should Fix issues exist

Append the following JSON on its own line at the very end of your output:

```json
{"verdict": "approve|request-changes|comment", "reason": "Brief one-line summary of verdict rationale"}
```

Example verdicts:
- `{"verdict": "approve", "reason": "Clean diff with no critical or important issues"}`
- `{"verdict": "request-changes", "reason": "SQL injection vulnerability in pkg/db/user.go"}`
- `{"verdict": "comment", "reason": "Missing error wrapping in 3 functions, no security issues"}`

### Step 8: Manual Review (All Projects)
...
**Reminder**: Only flag issues in code changed by the PR diff, not pre-existing issues in unchanged code.
